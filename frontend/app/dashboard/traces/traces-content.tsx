          ...
          <div className="space-y-4">
            {analysesWithTraces.map(({ analysis, traces }) => {
              ...
              const traceEvents = traces.map((t) => ({
                step: t.step_number,
                name: t.step_name,
                status: t.step_status as "processing" | "completed" | "failed",
                payload: (t.payload ?? {}) as Record<string, unknown>,
                timestamp: t.created_at,
              }));

              return (
                <Card key={analysis.id} className="bg-card/50 backdrop-blur-xl border border-border">
                  <CardHeader>
                    <div
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <CardTitle className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-lg font-bold">Analysis from {formatDate(analysis.created_at)}</span>
                        </CardTitle>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-2xl font-bold text-primary">{traces.length}</div>
                        </div>
                      </div>
                    </div>
                  </CardHeader>
