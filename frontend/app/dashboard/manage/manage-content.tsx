...
import { FileText, Settings, Archive, Trash2, Loader2, Upload, MoreVertical, FileBarChart, FileSpreadsheet, BookOpen } from "lucide-react";
...
import { extractPaystubFromPath, extractRsuFromPath, extractPolicyFromPath } from "@/actions/backend";
...
export function ManageContent({ documents, analyses }: ManageContentProps) {
  ...
  const [extractionType, setExtractionType] = useState<"paystub" | "rsu" | "policy" | null>(null);
  const [extractionSidebarOpen, setExtractionSidebarOpen] = useState(false);
  const [showActionsMenu, setShowActionsMenu] = useState<string | null>(null);

  const handleExtract = async (path: string, type: "paystub" | "rsu" | "policy") => {
    setExtracting(path);
            ...
            <CardContent className="space-y-6">
              ...
              >
                ...
                <label htmlFor="quick-upload" className="cursor-pointer">
                  <motion.div animate={isDragging ? { scale: 1.05 } : { scale: 1 }}>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Drag & drop or{" "}
                      <span className="text-primary font-medium">browse</span> PDFs
                    </p>
                  </motion.div>
                </label>
              </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <div className="relative">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setShowActionsMenu(showActionsMenu === doc.path ? null : doc.path)}
                            ...
                            {showActionsMenu === doc.path && (
                              ...
                              >
                                <div className="space-y-1">
                                  <button
                                    onClick={() => handleExtract(doc.path, "paystub")}
                                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-primary/10 text-left transition-colors"
                                  >
                                    <FileBarChart className="w-4 h-4 text-primary" />
                                    <span className="text-sm font-medium">Extract Paystub</span>
                                  </button>
                                  <button
                                    onClick={() => handleExtract(doc.path, "rsu")}
                                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-primary/10 text-left transition-colors"
                                  >
                                    <FileSpreadsheet className="w-4 h-4 text-primary" />
                                    <span className="text-sm font-medium">Extract RSU Grant</span>
                                  </button>
                                  <button
                                    onClick={() => handleExtract(doc.path, "policy")}
                                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-primary/10 text-left transition-colors"
                                  >
                                    <BookOpen className="w-4 h-4 text-primary" />
                                    <span className="text-sm font-medium">Extract Policy</span>
                                  </button>
                                </div>
                              </motion.div>
                            )}
