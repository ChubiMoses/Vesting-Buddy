    ...
    <div className="flex gap-6">
      ...
      <div className="flex flex-col justify-between h-28 py-0.5">
        {yAxisValues.reverse().map((value, i) => (
          <div key={i} className="text-[10px] text-muted-foreground font-mono tabular-nums">
        ...
        {ordered.map((a, i) => (
          ...
          >
            ...
            {hoveredIndex === i && (
              ...
              >
                <p className="text-xs font-semibold">${a.totalSavings.toLocaleString()}</p>
            ...
            <div className="h-28 w-full flex items-end">
              <motion.div
                ...
                className={cn(
                  ...
                  hoveredIndex === i ? "opacity-100 shadow-lg shadow-primary/50" : "opacity-70"
                )}
              />
            </div>
            <span className="text-[10px] text-muted-foreground truncate w-full text-center">
              {new Date(a.date).toLocaleDateString("en-US", { month: "short", year: "2-digit" })}
            </span>
          </div>
        ))}
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {nudges.map((nudge, index) => (
                ...
                >
                  <AgentNudgeCard 
                    nudge={nudge} 
                    onViewDetails={latest ? () => handleViewDetails(latest) : undefined}
                  />
                </motion.div>
              ))}
            </div>
            ...
            {savedAnalyses.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-muted-foreground text-sm mb-4 max-w-sm">
                  Upload your documents and run your first analysis to see insights here.
                </p>
                <button
                  type="button"
                  onClick={() => router.push("/dashboard/upload")}
